...
def analyze_feedback_budget
    ...
    if allergy(suggestion)
      puts "I see, I see. How about #{Clothing.allergy_confirmed(suggestion).name}? They're made of #{Clothing.allergy_confirmed(suggestion).material} instead and only cost $#{Clothing.allergy_confirmed(suggestion).price}."
    else
...
def analyze_feedback_luxury
    ...
    if allergy(suggestion)
      puts "I see, I see. How about #{Clothing.allergy_confirmed(suggestion).name}? They're made of #{Clothing.allergy_confirmed(suggestion).material} instead. They're worht $#{Clothing.allergy_confirmed(suggestion).price}."
    else
...
def exit_or_retry?
  ...
  prompt.select("Do you want to go again, or are you done for now?") do |menu|
    ...
    menu.choice 'Choice is a prison, free me.', -> {analyze_feedback_random}
...
def analyze_feedback_random
    sleep(1)
    puts "Maybe you're having a hard time choosing? May I throw out a suggestion?"
    sleep(1)
    puts "."
    sleep(1)
    puts ".."
